...
type KoiCreationBottomSheetProps = {
  isOpen: boolean;
  onClose: () => void;
  onKoiCreated: () => void;
};

const KoiCreationBottomSheet: React.FC<KoiCreationBottomSheetProps> = ({
  isOpen,
  onClose,
  onKoiCreated,
}) => {
  ...
  useEffect(() => {
    if (ponds.length > 0) {
      setKoiData(prev => ({ ...prev, pond_id: ponds[0].id }));
    }
  }, [ponds]);
  ...
  const handleSubmit = async () => {
    ...
    if (error) {
      console.error('Lỗi khi tạo koi:', error);
      Alert.alert('Lỗi', 'Đã xảy ra lỗi khi tạo koi.');
    } else if (data) {
      onKoiCreated();
      onClose();
    }
        ...
        <View style={{ padding: 16 }}>
          ...
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginTop: 16 }}>
            <Button onPress={handleSubmit} disabled={uploading} style={{ flex: 1, marginRight: 8 }}>
              <Text style={{ color: 'white', fontWeight: 'bold' }}>Tạo Koi</Text>
            </Button>
...
export default KoiCreationBottomSheet;
